...
export default function Montage() {
  ...
  return (
    <ThreePanelLayout
      ...
      inspector={<PlanningInspector kind="montage" events={events} selection={selection} onDelete={async (id) => {
            ...
            <div key={d.iso} className={`day-cell ${d.weekday === 0 || d.weekday === 6 ? 'muted' : ''}`} style={{ border: '1px solid var(--border)', padding: 8, borderRadius: 6 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                ...
                <button
                  ...
                  onClick={() => selectedOrderId && openModal({ type: 'assignment', orderId: selectedOrderId, kind: 'montage', date: d.iso })}
                >
                  Planen
                </button>
              </div>
              <div style={{ display: 'grid', gap: 4 }}>
                {(eventsByDate.get(d.iso) ?? []).map((ev) => (
                  <button key={ev.id} className="badge" style={{ display: 'grid', gap: 2, textAlign: 'left' }} onClick={() => select({ type: 'event', id: ev.id })}>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>#{ev.order_id}</span>
                      <span>{Math.round(ev.total_minutes/60)}h</span>
                    </div>
                    <div style={{ fontSize: 12 }}>Reise: {ev.travel_minutes} min</div>
